...
form.addEventListener('submit',async(e)=>{
  if(!form.action || form.action.includes('FORM_ENDPOINT_PLACEHOLDER')){
    e.preventDefault();
  ...
  try{
    ...
    statusEl.textContent='נרשמת בהצלחה. נשלח את פרטי המפגש כשיפורסמו ♡';
  }catch(err){statusEl.className='form-status error';statusEl.textContent='לא הצלחנו לשלוח. נסו שוב בעוד רגע.'}
  finally{button.disabled=false;button.innerHTML='אני רוצה להצטרף <span>←</span>'}
});
